...
(function (exports) {
   ...
   *
   * @private static
   * @attribute itemscope (+hidden) designates a repeating element (the clone source).
   * @attribute itemref can provide a value identifying a nested template's data source.
   ...
   * TODO: Evaluate if "concat" could be the normal procedure.
   *
   * @private static
   ...
   * TODO: Provide for nested expressions (the pattern captures all, so split(':') and apply).
   *
   * @private static
   * @property proxy will match the token-identifying wrapper (e.g. <p class="{{ style }}">{{ content }}</p>).
   ...
   * @property orsplit will progressively look for an existing value (e.g. "{{ perhaps|maybe|probably }}").
   */
  var PATTERN = {
    proxy: /{{\s*\b(.*)\b\s*}}/,
    notation: /[\.\[\]]+/,
  ...
  var clear = function (template, callback) {
    ...
    /* TODO: Consider as smarter utility that might preserve untemplated content. */
  ...
  var renderNestedTemplate = function (node, data, withHeritage) {
    ...
    if (!!deepdata) {
      if (deepdata === Object(deepdata)) {
        if (!!modifier && modifier === MODIFIER.forin) {
          renderObject(node, deepdata, withHeritage, true);
        } else {
   ...
   * @param {Boolean} withHeritage (optional) whether to include all inheritance.
   * @param {Boolean} forin (optional) whether to iterate over every property (true) else once per object.
   * @returns {null}
   */
  var renderObject = function (template, obj, withHeritage, forin) {
   ...
   * @param {Boolean} withHeritage (optional) whether to include all inheritance (this can be carried).
   * @returns {null}
   */
  ...
  var renderCollection = function (template, collection, withHeritage) {
    ...
    for (var i = 0; i < collection.length; i++) {
      ...
      traverse(clone, function (node) {
        var proxy, value, attrs, alternates, notation, formatter;
        if (isTemplate(node)) {
          renderNestedTemplate(node, model, withHeritage);
  ...
  exports.MicrodataTemplate = function (options) {
    ...
    api.refresh = function (ele, data, withHeritage) {
      renderNestedTemplate(ele, data, withHeritage);
    };
