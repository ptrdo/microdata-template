...
(function (exports) {
  ...
  var renderNestedNode = function (node, data, withHeritage) {
    ...
    var modifier, deepdata;
    if (!!proxy) {
      proxy = proxy[2].trim();
      modifier = proxy.match(PATTERN.expression);
      if (!!modifier) {
        proxy = modifier[3].trim();
        modifier = modifier[1].trim();
      }
      if (proxy in data) {
        deepdata = data[proxy];
      }
  ...
  var renderObject = function (template, obj, withHeritage, forin) {
    ...
    for (var key in obj) {
      if (!!forin) {
        clone = parent ? template.cloneNode(true) : template
      }
      ...
      traverse(clone, function (node) {
        ...
        if (node.nodeType === 3) {
          ...
          if (node.hasAttribute(MARKER.repeat) && node.hasAttribute(MARKER.source)) {
            renderNestedNode(node, model, withHeritage);
          } else {
            for (var j = attrs.length - 1; j >= 0; j--) {
              if (!!attrs[j].value) {
                proxy = attrs[j].value.match(PATTERN.proxy);
